...
void dijkstraByMinHeap(int start) {
//    一系列初始化
    Path.clear();
...
//    创建最小堆
    ...
    Util::MinHeap<Utilt::distance> distMinHeap(tem, countNodes);

    for(int i = 1; i <= countNodes; i++){

        // 找出未访问的最小距离的结点(通过最小堆)
        minDist = distMinHeap.getMin().getDist();
        k = distMinHeap.getMin().getIndex();
        distMinHeap.RemoveMin();
